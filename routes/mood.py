...
def create_mood(payload: MoodCreate, db: Session = Depends(get_db)):
    mood = models.Mood(user_id=payload.user_id, mood=payload.mood, note=payload.note)
    db.add(mood)
    db.commit()
    db.refresh(mood)
    return {"id": mood.id, "mood": mood.mood, "timestamp": mood.timestamp}
...
def latest_mood(user_id: str, db: Session = Depends(get_db)):
    ...
    return {"mood": latest.mood, "suggestions": suggestion_for_mood(latest.mood)}
